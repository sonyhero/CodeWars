...
function frogContest(n) {
  ...
  for (let i = 0; i <= Math.floor(cris / 2); i++) {
    tom += i
  }
  for (let i = 0; i <= cris + tom; i++) {
    cat += i
  }
  return `Chris ate ${cris} flies, Tom ate ${tom} flies and Cat ate ${cat} flies`
}

console.log('value', frogContest(5))